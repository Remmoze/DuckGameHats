...
const slowAES = {
    ...
    aes: {
        ...
        Rcon: [
            ...
            ,
        ...
        expandKey: function (key, size) {
            ...
            while (currentSize < expandedKeySize) {
                ...
                for (var k = 0; k < 4; k++) t[k] = expandedKey[currentSize - 4 + k];
        ...
        createRoundKey: function (expandedKey, roundKeyPointer) {
            var roundKey = [];
            for (var i = 0; i < 4; i++)
                for (var j = 0; j < 4; j++) roundKey[j * 4 + i] = expandedKey[roundKeyPointer + i * 4 + j];
            return roundKey;
        },
        ...
        subBytes: function (state, isInv) {
            for (var i = 0; i < 16; i++) state[i] = isInv ? this.rsbox[state[i]] : this.sbox[state[i]];
            return state;
        },

        /* iterate over the 4 rows and call shiftRow() with that row */
        shiftRows: function (state, isInv) {
            for (var i = 0; i < 4; i++) state = this.shiftRow(state, i * 4, i, isInv);
            return state;
        },
        ...
        shiftRow: function (state, statePointer, nbr, isInv) {
            for (var i = 0; i < nbr; i++) {
                if (isInv) {
                    ...
                    for (var j = 3; j > 0; j--) state[statePointer + j] = state[statePointer + j - 1];
                    state[statePointer] = tmp;
                } else {
                    var tmp = state[statePointer];
                    for (var j = 0; j < 3; j++) state[statePointer + j] = state[statePointer + j + 1];
        ...
        main: function (state, expandedKey, nbrRounds) {
            ...
            for (var i = 1; i < nbrRounds; i++) state = this.round(state, this.createRoundKey(expandedKey, 16 * i));
            state = this.subBytes(state, false);
            state = this.shiftRows(state, false);
        ...
        encrypt: function (input, key, size) {
            ...
            for (var i = 0; i < 4; i++ /* iterate over the columns */)
                for (var j = 0; j < 4; j++ /* iterate over the rows */) block[i + j * 4] = input[i * 4 + j];
            ...
            for (var k = 0; k < 4; k++ /* unmap the block again into the output */)
                for (var l = 0; l < 4; l++ /* iterate over the rows */) output[k * 4 + l] = block[k + l * 4];
            return output;
        },
        ...
        if (bytesIn !== null) {
            for (var j = 0; j < Math.ceil(bytesIn.length / 16); j++) {
                ...
                if (mode == this.modeOfOperation.CFB) {
                    ...
                    for (var i = 0; i < 16; i++) ciphertext[i] = byteArray[i] ^ output[i];
                    for (var k = 0; k < end - start; k++) cipherOut.push(ciphertext[k]);
                    input = ciphertext;
                    ...
                    for (var i = 0; i < 16; i++) input[i] = byteArray[i] ^ (firstRound ? iv[i] : ciphertext[i]);
                    firstRound = false;
        ...
        if (cipherIn !== null) {
            for (var j = 0; j < Math.ceil(cipherIn.length / 16); j++) {
                ...
                if (mode == this.modeOfOperation.CFB) {
                    ...
                    for (let i = 0; i < 16; i++) byteArray[i] = output[i] ^ ciphertext[i];
                    for (var k = 0; k < end - start; k++) bytesOut.push(byteArray[k]);
                    input = ciphertext;
                } else if (mode == this.modeOfOperation.OFB) {
                    if (firstRound) {
                        output = this.aes.encrypt(iv, key, size);
                    ...
                    for (let i = 0; i < 16; i++) byteArray[i] = (firstRound ? iv[i] : input[i]) ^ output[i];
                    firstRound = false;
                    for (var k = 0; k < end - start; k++) bytesOut.push(byteArray[k]);
                    input = ciphertext;
                }
